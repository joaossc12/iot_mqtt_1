...
#define WIFI_PASSWORD "SENHA"      // Substitua pela senha da sua rede Wi-Fi
#define MQTT_SERVER "IP"                // Substitua pelo endereço do host - broket MQTT: Ex: 192.168.1.107
#define MQTT_USERNAME "YOUR_USERNAME"     // Substitua pelo nome da host MQTT - Username
#define MQTT_PASSWORD "SENHA HOST"     // Substitua pelo Password da host MQTT - credencial de acesso - caso exista
...
// Temporização da coleta de temperatura - how often to measure our temperatura
#define AR_WORKER_ATT_TIME 1
...
// VARIAVEIS GLOBAIS
//---------------------------------------------------
float setpoint_temp = 23; //Referencia de Temperatura
float temperatura = 23;  //Temperatura Interna
int32_t cortina = 0; //Percentual de abertura da cortina 
bool flag_lampada = false; //Flag global para estado da lâmpada
ssd1306_t ssd; // Display de 128 x 64



//Inicia e configura pinos para os periféricos
float real_temp_proporcional_control(float setpoint){
    static float old_output = 23; //valor inicial 
    float erro = setpoint - old_output; // calcula o erro entre referência e temperatura
    float kp = 0.2; //Ganho proporcional 
    float new_temp = (float)erro * kp + old_output; //Aplica o ganho no erro e soma a temperatura anterior 
    old_output = new_temp; //Atualiza saída
    return new_temp; //Retorna novo valor 
}
...
// Publicar temperatura do ar
static void publish_temperature_ar(MQTT_CLIENT_DATA_T *state);
...
static void temp_ar_worker_fn(async_context_t *context, async_at_time_worker_t *worker);
static async_at_time_worker_t temperature_ar_worker = { .do_work = temp_ar_worker_fn };
...
static void publish_temperature_ar(MQTT_CLIENT_DATA_T *state) {
    const char *temperature_key = full_topic(state, "/temp_out");
    temperatura = real_temp_proporcional_control(setpoint_temp);
    AtualizaDisplay();
...
static void sub_unsub_topics(MQTT_CLIENT_DATA_T* state, bool sub) {
    ...
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/lampada"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/cortina"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/setpoint_temp"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
}
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags) {
    ...
    if (strcmp(basic_topic, "/print") == 0) {
    ...
    } else if (strcmp(basic_topic, "/lampada") == 0){
        if (lwip_stricmp((const char *)state->data, "On") == 0 || strcmp((const char *)state->data, "1") == 0){
            ...
            gpio_put(LED_BLUE, 0);
            flag_lampada = 0;}
    } else if (strcmp(basic_topic, "/cortina") == 0){
        cortina = strtol((char *)state->data, NULL, 10);
        printf("Percentual da cortina: %s %% \n", (char *)state->data);
    } else if (strcmp(basic_topic, "/setpoint_temp") == 0){
        setpoint_temp = strtof((char *)state->data, NULL);
        printf("Temperatura do Ar: %s °C \n", (char *)state->data);
    }
    AtualizaDisplay();
}
...
static void temp_ar_worker_fn(async_context_t *context, async_at_time_worker_t *worker) {
    ...
    publish_temperature_ar(state);
    async_context_add_at_time_worker_in_ms(context, worker, AR_WORKER_ATT_TIME * 1000);
}
...
static void mqtt_connection_cb(mqtt_client_t *client, void *arg, mqtt_connection_status_t status) {
    ...
    if (status == MQTT_CONNECT_ACCEPTED) {
        ...
        // Publica após 1s Sempre!
        temperature_ar_worker.user_data = state;
        async_context_add_at_time_worker_in_ms(cyw43_arch_async_context(),&temperature_ar_worker, 0);
...
void AtualizaDisplay(void){
    ...
    snprintf(string_sp_ar, sizeof(string_sp_ar), "%.2f%%", setpoint_temp);
    ...
    ssd1306_draw_string(&ssd, string_ar, 65, 27);
    ssd1306_draw_string(&ssd, " C", 100, 27);
    ssd1306_draw_string(&ssd, "STPNT:", 5, 37);
    ssd1306_draw_string(&ssd, string_sp_ar, 65, 37);
    ssd1306_draw_string(&ssd, " C", 100, 37);
    ssd1306_send_data(&ssd);
}